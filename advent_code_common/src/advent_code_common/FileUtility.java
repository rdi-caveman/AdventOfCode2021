...
public class FileUtility {
	...
	public static List<Integer> readListOfInteger(String filename) {
		...
		try (Stream<String> stream = Files.lines(Paths.get(filename))) {
			ints = stream.map(s -> s.isEmpty() ? null : Integer.parseInt(s)).collect(Collectors.toList());
		} catch (IOException e) {
	...
	public static int[] readArrayOfInt(String filename) {
		List<String> input = readListOfString(filename);
		int length = input.size();
		int[] returnArray = new int[length];
		IntStream.range(0,length)
			.forEach(i -> {returnArray[i] = Integer.valueOf(input.get(i));});
		return returnArray;	
	}	
}
