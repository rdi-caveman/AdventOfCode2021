...
public class Day13
{
	...
	private static void displayCode() {
		OptionalInt maxX = paper.keySet().stream()
				.mapToInt(p -> p.x)
				.max();
		OptionalInt maxY = paper.keySet().stream()
				.mapToInt(p -> p.y)
				.max();
	...
	public static void initialize() {
		String input[];
		try {
			input = (new String(Files.readAllBytes(Paths.get(DAY13_INPUT_TXT)))).split("\r?\n\r?\n");
			Arrays.stream(input[0].split("\\s+"))
				.map(s -> s.split(","))
