...
public class Day14 {
	...
	private static Map<String, Pair<String, String>> rulePairs = new HashMap<>();  // rule ab -> c becomes rulePair ab -> (ac, ca)
		...
		pairs.entrySet().stream()
			.forEach(e -> {
				if (rulePairs.containsKey(e.getKey())) {
					result.put(rulePairs.get(e.getKey()).first, result.getOrDefault(rulePairs.get(e.getKey()).first,0L)+e.getValue());
					result.put(rulePairs.get(e.getKey()).second, result.getOrDefault(rulePairs.get(e.getKey()).second,0L)+e.getValue());
					elementCount.put(rules.get(e.getKey()),elementCount.getOrDefault(rules.get(e.getKey()),0L)+e.getValue());
				}
				else {
	...
	private static void initialize() {	
		String[] input;
		try {
			input = (new String(Files.readAllBytes(Paths.get(DAY14_INPUT_TXT)))).split("\r?\n\r?\n");
			for (int i=1; i<input[0].length(); i++) {
				pairs.put(input[0].substring(i-1,i+1), pairs.getOrDefault(input[0].substring(i-1,i+1),0L)+1);
			}
			for (int i=0; i<input[0].length(); i++) {
				elementCount.put(input[0].substring(i,i+1), elementCount.getOrDefault(input[0].substring(i,i+1),0L)+1);
			}
